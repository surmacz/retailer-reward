...
export const handlers = [
  rest.get('/purchases', (req, res, ctx) => {
    const clientsObj = clients.reduce(
    ...
    return res(
      ...
      ctx.json(
        purchases.map((purchase) => ({
          ...purchase,
          ...Object.keys(clientsObj[purchase.clientId]).reduce(
            (acc, clientKey) => ({
              ...acc,
              ['client' + clientKey[0].toUpperCase() + clientKey.substring(1)]:
                clientsObj[purchase.clientId][clientKey],
            }),
            {}
          ),
        }))
          ...
          .map((purchase) => ({
            ...
            ...Object.keys(client).reduce(
              (acc, clientKey) => ({
                ...acc,
                ['client' +
                clientKey[0].toUpperCase() +
                clientKey.substring(1)]: client[clientKey],
              }),
              {}
            ),
          }))
